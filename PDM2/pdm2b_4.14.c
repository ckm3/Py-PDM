...
int pdm2( int ne, double datx[], double daty[], double sig[], char root[31], int write );
...
/*------- command line use--------------
	 ...
	 pdm2 root Per
	 pdm2 root Per1 Per2
	 pdm2 root Nrange Nstep
	 pdm2 root Per Nrange Nstep
	 pdm2 root Per1 Per2 Nrange Nstep
---------------------------------------*/
...
int main( int argc, char *argv[] )
{
    ...
    /* in the line below, change ".bin" to your preferred extension for input file */
    sprintf(file,"%s.bin",root);

	//sprintf( file, "v353lyr.csv" );   //<<<==============test=====================<<<

    printf("Attempting to open data file %s\n",file);
    
    /*  read the data here   */
	/*  the second arg in read_file() is the number of title lines (discarded)  */
	...
	/*---check for Blazhko cases,---*/
	/*---skip rough cut if periods have been specified ---*/
	if( argc == 4 && atof(argv[2]) > 100. ) {
		/*  user has specified Blazhko params only  */
        Blazhko = TRUE;
		Nrange = atoi(argv[2]);
		Nstep = atoi(argv[3]);
	}
	else if( argc == 5 ) {
        /* user has specified a single trial period; plus Blazhko params */
        ptry = atof(argv[2]);
        ftry = 1./ptry;
        minf0 = ftry - 0.5;
        if (minf0 < 0.) {
        ...
        /* user has specified a lower & upper period limit; plus Blazhko params */
        ptry = atof(argv[2]);
        minf0 = 1./ptry;
        ...
        /* user has specified a single trial period; set a frequency range */
        ptry = atof(argv[2]);
        ftry = 1./ptry;
        minf0 = ftry - 0.5;
        if (minf0 < 0.) {
    ...
    else if (argc == 4 && atof(argv[2]) < 100 ) {
        /* user has specified a lower & upper period limit; set frequencies accordingly */
        ptry = atof(argv[2]);
        minf0 = 1./ptry;
    ...
    /*  rough cut over the data, do not write results to pdm2b.out  */
    
    lpoints = 3;
    invert_curve = FALSE;
    segdev = 500;               /*  500 to get some segs, maybe?  */
    
    sprintf(out,"%s",root);
    pdm2( Numdat, times, mags, sigs, out, 0);

    
    /*    printf("Please press any key to continue\n");
    getchar(); */
    
    /*  now zero in on the best candidate and write results to pdm2b.out */
    
    lpoints = 500;
    minf0 = fthmin[1]-0.02;
    maxf0 = fthmin[1]+0.02;
    invert_curve = FALSE;
    segdev = 5000;        /*  5000 to get a single segment per CSTAR season */
    
    /*  do_non_par = TRUE;             Monte Carlo analysis  */
    /*  nb1 = 500;                    to check small values  */
    
    /*  set for variable period search...might be a little slow.... */
    do_beta_scan = FALSE;
    beta_min = -10.;
    beta_max = +10.;            /* set these two to do a period change case  */
    nb0 = 201;                  /* optional fine scan for period change      */
    
    /*=====================Blazhko Analysis===================================*/
    
    //Blazhko = TRUE;             /*  comment out for normal run or command line use */
    
    /*  normal run  */
    if( !Blazhko ) {
        pdm2( Numdat, times, mags, sigs, out, 1);
    }
    else {
        /*  Blazhko scan of subsets of the data, also can be used for variable period  */
        printf( "Start Blazhko analysis\n" );
        fb = fopen( "Blaz_sum.dat", "w" );
        ...
        fprintf( fb, "# Nstep= %d , Nrange= %d , segdev= %d\n", Nstep, Nrange, segdev );
        fprintf( fb, "# Time Amp Min Max Mean Period N\n" );	
        /*  analysis parameters for subsequences  */
        lpoints = 100;
        invert_curve = TRUE;
        
        n3 = (Numdat-1-Nrange/2)/Nstep;
        printf( "\n\nBlazhko: %d Subsegs will be analyzed\n", n3 );
        for( i = 1; i <= n3; i++ ) {
            n1 = (i-1)*Nstep;
            n2 = n1 + Nrange;
            sprintf( out, "B_%d", i );
            
            pdm2( Nrange, times+n1, mags+n1, sigs+n1, out, 1);
            
            fprintf( fb, "%g %g %g %g %g %.8g %d\n", xmean, yamp, ymin, ymax, ymean_mean, 1./fthmin[1], nplot );
        }
        fclose( fb);
    }
}
